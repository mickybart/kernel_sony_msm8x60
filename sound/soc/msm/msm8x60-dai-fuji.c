...
static struct snd_soc_dai_driver msm_pcm_compr_dais[] = {
{
	.name = "msm-compr-dai",
	.compress_dai = 1,
	.playback = {
...
static __devinit int asoc_msm_compr_probe(struct platform_device *pdev)
{
	dev_info(&pdev->dev, "%s: dev name %s\n", __func__, dev_name(&pdev->dev));
	return snd_soc_register_dai(&pdev->dev, msm_pcm_compr_dais);
}

static int __devexit asoc_msm_dai_remove(struct platform_device *pdev)
{
...
static struct platform_driver asoc_msm_codec_driver = {
	...
	.remove = __devexit_p(asoc_msm_dai_remove),
	.driver = {
			.name = "msm-codec-dai",
			.owner = THIS_MODULE,
	},
...
static struct platform_driver asoc_msm_compr_driver = {
	.probe = asoc_msm_compr_probe,
	.remove = __devexit_p(asoc_msm_dai_remove),
	.driver = {
			.name = "msm-compr-dai",
			.owner = THIS_MODULE,
...
{
	platform_driver_register(&asoc_msm_codec_driver);
	platform_driver_register(&asoc_msm_cpu_driver);
	platform_driver_register(&asoc_msm_compr_driver);
	return 0;
}
...
static void __exit msm_dai_exit(void)
{
	...
	platform_driver_unregister(&asoc_msm_compr_driver);
}
